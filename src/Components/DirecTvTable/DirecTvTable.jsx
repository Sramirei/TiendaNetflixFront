...
const DirecTvTable = () => {
    ...
    useEffect(() => {
      ...
      axios
        .get(process.env.REACT_APP_API_URL + 'directv', {
          headers: {
      ...
      try {
        ...
        if (result.isConfirmed) {
          ...
          const response = await axios.post(
            process.env.REACT_APP_API_URL + 'directv',
            accountData,
            {
              headers: {
                Authorization: `Bearer ${session.token}`, // Agrega el token de sesión en los encabezados con el formato "Bearer {token}"
    ...
    const editAccount = async (accountId) => {
      try {
        ...
        const response = await axios.get(
          `${process.env.REACT_APP_API_URL}directv/${accountId}`,
          {
            headers: {
              Authorization: `Bearer ${session.token}`, // Agrega el token de sesión en los encabezados con el formato "Bearer {token}"
            },
    ...
    const deleteAccount = async (accountId) => {
      try {
        ...
        if (result.isConfirmed) {
          try {
            const response = await axios.delete(`${process.env.REACT_APP_API_URL}directv/${accountId}`, {
              headers: {
                Authorization: `Bearer ${session.token}`, // Agrega el token de sesión en los encabezados con el formato "Bearer {token}"
              },
            });
