...
const Navigation = ({session}) => {
  ...
  <li><a onClick={() => handleNavbarClick(<Card session={session}/>)} className="active" >Servicios</a></li>
  {session.perfil == 1 ? (
    <>
    <li><a onClick={() => handleNavbarClick(<UserTable session={session}/>)}>Usuario</a></li>
      <li><a onClick={() => handleNavbarClick(<SalesTable session={session}/>)}>Ventas General</a></li>
      <li><a onClick={() => handleNavbarClick(<SalesUserTable session={session} />)}>Compras</a></li>
      <li><a onClick={() => handleNavbarClick(<NetflixTable session={session} />)}>Netflix</a></li>
      <li><a onClick={() => handleNavbarClick(<AmazonTable session={session} />)}>Amazon</a></li>
      <li><a onClick={() => handleNavbarClick(<DisneyTable session={session} />)}>Disney</a></li>
      <li><a onClick={() => handleNavbarClick(<PlexTable session={session} />)}>Plex</a></li>
      <li><a onClick={() => handleNavbarClick(<YoutubeTable session={session} />)}>Youtube</a></li>
      <li><a onClick={() => handleNavbarClick(<CrunchyrollTable session={session} />)}>Crunchyroll</a></li>
      <li><a onClick={() => handleNavbarClick(<ParamountTable session={session} />)}>Paramount</a></li>
